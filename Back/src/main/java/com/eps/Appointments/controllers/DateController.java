...
public class DateController {
     ...
     **/
    @PostMapping
    private ResponseEntity<? extends Object> create(@RequestBody MedicalDateDTO dateDTO) {
        try {
            MedicalDateDTO newDate = dateService.create(dateDTO);
            if (newDate != null) {
    ...
    /**
     * A method is created which can obtain this information and say if the admin
     * can or not by means of id
     * 
    ...
    public ResponseEntity<? extends Object> getById(@PathVariable("id") Integer id) {
        try {
            MedicalDateDTO date = dateService.getById(id);
            if (date != null) {
    ...
    /**
     * a method is created so that you can check if the information is being
     * obtained
     * 
     * @GetMapping is a compound annotation that acts as a shortcut
     *             for @RequestMapping
     ...
     **/
    @PutMapping("/{id}")
    private ResponseEntity<? extends Object> Update(@RequestBody MedicalDateDTO dateDTO,
            @PathVariable("id") String id) {
        try {
            MedicalDateDTO updatedDate = dateService.updateDate(dateDTO);
            if (updatedDate != null) {
